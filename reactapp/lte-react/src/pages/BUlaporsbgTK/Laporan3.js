...
class Laporan3 extends React.Component 
{ 
    ...
    async componentDidMount (){
        const res = axios.get('http://127.0.0.1:8000/api/laporsbgtk/laporanku');
        console.log(res);
        if ((await res).data.stt === 200) {
            this.setState({
                laporan: (await res).data.LaporsbgTKku,
                loading: false,
    ...
    changestatus = async (e,id) => {
        ...
        const res = await axios.put(`http://127.0.0.1:8000/api/laporsbgtk/verifikasilaporan/${id}` , this.state);
        if(res.status ===200)
        {
    ...
    deleteLaporan =  async (e,id) => {
        ...
        const res = await axios.delete(`http://127.0.0.1:8000/api/laporsbgtk/deletelaporan/${id}`);
        if( res.data.stt === 200)
        {
            ...
            this.state.laporan.map( (item) => {
                return(
                    <tr key={item.id}>
                        ...
                        <td>{item.besar_data_pembanding}</td>
                        <td>{item.bulan_rujukan_MF}</td>
                        <td>{item.data_MF}</td>
                        <td>{item.selisih}</td>
                        <td>{item.tanggal_canvasing_PTSDK}</td>
                ...
                <div className='row'>
                    <div className='col'>
                        <div className='card'>
                            ...
                            <br/>
                            <br/>
                                <table className='table table-bordered table-striped table-fixed table-responsive' responsive>
                                    <thead>
                                        <tr>
                                            <th>ID</th>
                                            <th>Kode BU</th>
                                            ...
                                            <th>Besaran Data Pembanding</th>
                                            <th>Bulan Rujukan MF</th>
                                            <th>Data MF</th>
                                            <th>Selisih</th>
                                            <th>Tanggal Canvasing PTSDK</th>
...
export default withRouter(Laporan3);